...
def state(tmp_path, monkeypatch):
    ...
    for number in range(7):
        runtime.remember(f"memory {number}", f"receipt {number}")
...
def test_configured_command_delivers_identity_handoff_and_older_memories(state, source):
    ...
    assert (REPO / "SEREIN.md").read_text() in content
    assert "Observe an actual host lifecycle event" in content
    assert "memory 0" in content  # Older than the ordinary briefing's five-memory window.
...
def test_prompt_hook_rechecks_state_and_blocks_tampering(state):
    assert invoke("UserPromptSubmit")["continue"] is True
